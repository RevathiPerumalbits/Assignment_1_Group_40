...
def evaluate_and_register_models(trained_models, X_train):
    ...
    logger.info(
        f"Evaluating models with MLflow tracking URI: {mlflow.get_tracking_uri()}"
    )
    ...
    mlflow.sklearn.log_model(
        best_model, artifact_path=best_model_name, input_example=input_example
    )

    logger.info(
        f"Best model ({best_model_name}) registered with URI: {model_uri}, saved to {save_path}"
    )
